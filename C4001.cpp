...
nts::Tristate nts::C4001::compute(std::size_t pin)
{
    if (pin == 3 || pin == 4 || pin == 10 || pin == 11) {
        ...
        _valMap[pin - 1] = gate::Nor(_links[_valOut[pin].first - 1]->getValue(_valOut[pin].first), _links[_valOut[pin].second - 1]->getValue(_valOut[pin].second));
...
void nts::C4001::setLink(std::size_t pin, nts::IComponent &other, std::
size_t otherPin)
{
    if (pin >= 1 && pin <= 14) {
        if (pin == 14 || pin == 7) {
...
nts::Tristate nts::C4001::getValue(std::size_t pin)
{
    return (_valMap[pin - 1]);
}

void nts::C4001::dump(const std::string &name) 
{
    for (int i = 0 ; i < 14 ; i++) {
        if (i != 14 && i != 7 && i != 0) {
...
void nts::C4001::setValue(const std::string &value) {}