...
nts::Tristate nts::C4030::compute(std::size_t pin)
{
    if (pin == 3 || pin == 4 || pin == 10 || pin == 11) {
        ...
        _valMap[pin - 1] = gate::Xor(_links[_valOut[pin].first - 1]->getValue(_valOut[pin].first), _links[_valOut[pin].second - 1]->getValue(_valOut[pin].second));
...
void nts::C4030::setLink(std::size_t pin, nts::IComponent &other, std::
size_t otherPin)
{
    if (pin >= 1 && pin <= 14) {
        if (pin == 14 || pin == 7) {
            std::cout << "ERROR PIN 4030" << std::endl;
            exit(84);
        }
        _link[pin] = otherPin;
        _links[pin - 1] = &other;
...
nts::Tristate nts::C4030::getValue(std::size_t pin)
{
    return (_valMap[pin - 1]);
}

void nts::C4030::dump(const std::string &name) 
{
    for (int i = 0 ; i < 14 ; i++) {
        if (i != 14 && i != 7 && i != 0) {
...
void nts::C4030::setValue(const std::string &value) {}
